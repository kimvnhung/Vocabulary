...
public class NouveauMot implements Parcelable{
    ...
    public NouveauMot(int _expert_point) {
    ...
    public static void setColorExpert(View view,int expertPoint){
       try {
           GradientDrawable gd = (GradientDrawable)view.getBackground().getCurrent();
           gd.setColor(Color.parseColor(getColorExpert(expertPoint)));
       }catch (Exception e){
            TextView para = (TextView)view;
           para.setTextColor(Color.parseColor(getColorExpert(expertPoint)));
       }
    }

    public static String getColorExpert(int point){
        String[] listColor={
                "#79f480",
                "#2dc937",
                "#99c140",
    ...
    public boolean equals(Object obj) {
        if (obj != null){
            if (obj instanceof NouveauMot){
                if (((NouveauMot) obj).get_id().equals(get_id()) &&
                        ((NouveauMot) obj).get_leMot().equals(get_leMot()) &&
                        ((NouveauMot) obj).get_type_word().equals(get_type_word()) &&
                        ((NouveauMot) obj).get_lv2().equals(get_lv2())){
                    return true;
                }
            }
        }
