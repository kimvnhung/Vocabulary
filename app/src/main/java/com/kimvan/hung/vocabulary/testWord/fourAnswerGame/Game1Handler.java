...
public class Game1Handler extends ScoreHandler{
    ...
    private int[] getMaxMinPoint(){ //result[0] là max, result[1] min
    ...
    private ArrayList<NouveauMot> getLeMot(Cursor c){
    ...
    private NouveauMot getMotNearRandomPoint(){
    ...
    private NormalWordMeaning getRandomMeaning(String id){
    ...
    //đưa ra nghĩa tiếng việt nếu anglaisouvietnamien là true và ngược lại
    private String getMeaning(NormalWordMeaning selected){
    ...
    //đổi lấy nghĩa tương ứng
    public String getSameMeaning(String lastMeaning){
        String result = "";
        try {
            SQLiteDatabase db = dbHandler.getWritableDatabase();
            //lấy tên cột của đáp án cũ
            //getAnglaisOuVietnamien trả về true thì nghĩa mới là en_Vietnamien, cột cũ là en_Anglais và ngược lại
            String columnName = getAnglaisOuVietnamien()?dbHandler.COLUMN_EN_ANGLAIS:dbHandler.COLUMN_EN_VIETNAMIEN;
            Cursor c = db.rawQuery("SELECT * FROM "+dbHandler.TABLE_NAME_MEANING+" WHERE "+
                    columnName+" = \'"+lastMeaning+"\'",null);
            c.moveToFirst();
            //đổi cột
            int columnNameAnother = columnName.equals(dbHandler.COLUMN_EN_VIETNAMIEN)
                    ?2:3;
            result = c.getString(columnNameAnother);
            c.close();
        }catch (Exception e){
            e.getMessage();
        }
